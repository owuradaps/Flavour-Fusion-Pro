...
def user_login(request):
    breadcrumbs = get_breadcrumbs([{"title": "Login"}])
    ...
    return render(
        request, "account/login.html", {"form": form, "breadcrumbs": breadcrumbs}
    )


def register(request):
    breadcrumbs = get_breadcrumbs([{"title": "Register"}])
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                return redirect(
                    reverse("account:user_profile", kwargs={"username": user.username})
                )
    ...
    return render(
        request, "account/register.html", {"form": form, "breadcrumbs": breadcrumbs}
    )


@login_required
def profile(request, username=None):
    breadcrumbs = get_breadcrumbs([{"title": "Profile"}])
...
def user_profile(request, username):
    breadcrumbs = get_breadcrumbs(
        [
            {
                "title": "User Profile",
                "url": reverse("account:user_profile", args=[username]),
            }
        ]
    )
